...
import { type IncidentDTO, type Severity, Status } from "../backend";
...
export function IncidentCard({ incident }: IncidentCardProps) {
  ...
  const [isOpen, setIsOpen] = useState(statusKey !== Status.resolved);
  const isResolved = statusKey === Status.resolved;
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <h3
              className={`font-semibold text-base leading-snug ${isResolved ? "text-muted-foreground" : "text-foreground"}`}
            >
              {incident.title}
            </h3>
          </div>
